...
class ContactsRouteFragment : BaseFragment<ContactsRouteViewModel, ContactsRouteState>() {
    override val vmClass = ContactsRouteViewModel::class.java
    ...
    override fun updateState(state: ContactsRouteState) {
        with(binding) {
            if (state.loading) {
                ...
                if (state.error != null) {
                    Toast.makeText(context, getString(R.string.route_not_found), Toast.LENGTH_LONG)
                        .show()
                }
            }
